...
describe("topaz stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "topaz-mcp-test", version: "0.1.0" });
    ...
    expect(names).toEqual(["check_pricing","get_task","login","upscale_image","upscale_video"]);
    ...
    for (const model of ["topaz-upscale-image","topaz-upscale-video"]) {
